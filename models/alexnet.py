...
class AlexNet(nn.Module):
    def __init__(self, n_classes=100, n_jigsaw_classes=30, is_rotation=False, dropout=True):
        ...
        self.jigsaw_classifier = nn.Linear(4096, n_jigsaw_classes)
    ...
    def forward(self, x, lambda_val=0):
        ...
        x = self.classifier(x)
        if self.is_rotation:
          return self.class_classifier(x), self.rotation_classifier(x)
        else:
          return self.class_classifier(x), self.jigsaw_classifier(x)

def alexnet(classes, jigsaw_classes, is_rotation):
    model = AlexNet(classes, jigsaw_classes, is_rotation)
    for m in model.modules():
        if isinstance(m, nn.Linear):
            nn.init.xavier_uniform_(m.weight, .1)
            nn.init.constant_(m.bias, 0.)

    state_dict = torch.load("/content/drive/MyDrive/MLAI_Project/alexnet_caffe.pth.tar")
