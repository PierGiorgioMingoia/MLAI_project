...
class Logger():
    ...
    def log(self, it, iters, losses, samples_right, total_samples,jigsaw_losses=None, jigsaw_samples_right=None):
        self.current_iter += 1
        ...
        if it % self.update_f == 0:
            print("%d/%d of epoch %d/%d %s - %s [bs:%d]" % (it, iters, self.current_epoch, self.max_epochs, loss_string,
                                                                acc_string, total_samples))   
             # update tf log
            if self.tf_logger:
                for k, v in losses.items(): self.tf_logger.scalar_summary("train/loss_%s" % k, v, self.current_iter)
        
        if jigsaw_losses!=None:
            jigsaw_loss_string = ", ".join(["%s : %.3f" % (k, v) for k, v in jigsaw_losses.items()])
            '''for k, v in jigsaw_samples_right.items():
                past = self.epoch_stats.get(k, 0.0)
                self.epoch_stats[k] = past + v
            self.total += total_samples'''
            jigsaw_acc_string = ", ".join(["%s : %.2f" % (k, 100 * (v / total_samples)) for k, v in jigsaw_samples_right.items()])
            if it % self.update_f == 0:
                print("%d/%d of epoch %d/%d %s - %s [bs:%d]" % (it, iters, self.current_epoch, self.max_epochs, jigsaw_loss_string,
                                                                    jigsaw_acc_string, total_samples))
